...
class DiscordClient(discord.Client):
    ...
    async def on_message(self, message):
        if message.author == client.user or message.content[
            0] != '!':  # if the message is the bot or does not have the '!' at the beginning then it is ignored
            return

        args = (message.content[
                1:]).lower().split()  # splits all arguments into a list lowercased. (!helLo woRld => ['hello', 'world'])
